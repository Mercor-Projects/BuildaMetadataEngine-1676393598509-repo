...
`open_video`.
"""
from utils import detect_blur,yolo
import csv
...
class SceneManager:
    ...
    def detect_scenes(self,
		...
		# vis=False)
                ...
                # print(FrameTimecode(position.frame_num,25))
            new_cuts = self._process_frame(position.frame_num, frame_im, callback)
            mean,blurry=detect_blur(frame_im)
            
                
            metadata[f'id-{position.frame_num}']={
                "Frame-No":position.frame_num,
                "Frame-TimeStamp":str(FrameTimecode(position.frame_num,video.frame_rate)),
                "Cut-Edit-Detection":new_cuts,
                "Blurry":f"{blurry} with a value of {mean}",
